...
typedef enum {
	LookingForType, LookingForClass,
	LookingForInterface, LookingForVariable, LookingForFunction
} reasonT;
...
public:
	...
	static void InterfaceNotImplemented(Decl *classDecl, AstType *intfType);


	// Errors used by semantic analyzer for identifiers
	static void IdentifierNotDeclared(Identifier *ident, reasonT whyNeeded);
	static void NewOpeartionTypeNotClass(Expr* exp, AstType* type);

	// Errors used by semantic analyzer for expressions
	static void IncompatibleOperand(Operator *op, AstType *rhs); // unary
	static void IncompatibleOperands(Operator *op, AstType *lhs, AstType *rhs); // binary
	...
	static void ArgMismatch(Expr *arg, int argIndex, AstType *given, AstType *expected);
	static void PrintArgMismatch(Expr *arg, int argIndex, AstType *given);


	// Errors used by semantic analyzer for field access
	static void FieldNotFoundInBase(Identifier *field, AstType *base);
	static void InaccessibleField(Identifier *field, AstType *base);


	// Errors used by semantic analyzer for control structures
	static void TestNotBoolean(Expr *testExpr);
	static void ReturnMismatch(ReturnStmt *rStmt, AstType *given, AstType *expected);
	...
	static void ForStmtInitMissFormat();
	static void ForStmtStepMissFormat();
	static void TypeMissOperation();
	static void TypeNotDecl();
	static void InheritSelf();
	static void InterfaceConflict(ClassDecl*, Decl*, Decl*);
