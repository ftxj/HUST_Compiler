...
class CodeGenerator {
	...
	public:
		...
		int nextLocalOffset; //contains the offset to use when assigning the next local variable. should update this each time we assign a local var. 
		int nextParamOffset; //same thing for param
		int nextGlobalOffset; //same thing for global variables
		int currFunctionFrameSize; 
		Segment currSeg; //equals fpRelative when in a function and gpRelative when not in a function. 
		std::map<std::string, Location*> *currParametersLocations; 
		int ReadWriteField; //initialized to -1. 
		//equals 0 if we are writing to the left side. 
		//equals 1 if we are reading from the right side. 
		CodeGenerator();
		...
		Location *GenLoadConstant(std::string str);
		Location *GenLoadLabel(std::string label);
		...
		// was stored.
		Location *GenBinaryOp(std::string opName, Location *op1, Location *op2);
		...
		Location *GenLCall(std::string label, bool fnHasReturnValue);
		...
		// return a value
		void GenIfZ(Location *test, std::string label);
		...
		void GenVTable(std::string className, std::list<std::string> *methodLabels);
